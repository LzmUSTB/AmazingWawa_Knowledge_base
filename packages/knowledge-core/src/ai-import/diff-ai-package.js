...
function generatedNoteContent(operation, packageFiles) {
  const format = noteFormatForOperation(operation, packageFiles);
  if (format === "html") return { format, path: generatedHtmlNotePath(operation), targetPath: targetHtmlNotePath(operation), html: packageFiles.generatedHtmlNoteFiles[generatedHtmlNotePath(operation)] || "", markdown: "" };
  if (format === "markdown") return { format, path: generatedMarkdownNotePath(operation), targetPath: targetMarkdownNotePath(operation), markdown: packageFiles.generatedNoteFiles[generatedMarkdownNotePath(operation)] || "", html: "" };
  return { format: "none", path: "", targetPath: "", markdown: "", html: "" };
}
...
function blockTypeSummary(raw = "", path = "") {
  ...
  return { type: parsed.type || path.split("/").pop()?.replace(/\.ya?ml$/i, ""), title: parsed.title || parsed.name || "", kind: parsed.kind || "", engine: parsed.renderer?.engine || "", sourcePath: path, example: parsed.example || parsed.examples?.[0] || null };
}

export function diffAiPackage(currentVault, validatedPackageOrFiles) {
  const validatedPackage = validatedPackageOrFiles?.normalizedOperations && validatedPackageOrFiles?.packageFiles ? validatedPackageOrFiles : validateAiPackage(currentVault, validatedPackageOrFiles);
  ...
  validatedPackage.normalizedOperations.forEach((operation) => {
    if (operation.type === "add_domain") {
      domainsToAdd.push({ id: operation.id, title: operation.title, description: operation.description || "", color: operation.color || "", order: operation.order, reason: "domain creation from package" });
      if (!filesToModify.some((file) => file.path === "domains.yaml")) filesToModify.push({ path: "domains.yaml", kind: "domains" });
      if (!currentVault.vault?.defaultDomain && !filesToModify.some((file) => file.path === "vault.yaml")) filesToModify.push({ path: "vault.yaml", kind: "vault" });
    }
    if (operation.type === "add_node") {
      const metaPath = targetMetaPath(operation);
      const note = generatedNoteContent(operation, packageFiles);
      const containsEdge = { id: edgeId(operation.parentId, "contains", operation.id), from: operation.parentId, to: operation.id, relation: "contains" };
      filesToCreate.push({ path: metaPath, sourcePath: generatedMetaPath(operation), kind: "meta" });
      if (note.targetPath) filesToCreate.push({ path: note.targetPath, sourcePath: note.path, kind: note.format === "html" ? "note-html" : "note" });
      nodesToAdd.push({ id: operation.id, title: operation.title, domain: operation.domain, type: operation.nodeType, status: operation.status, parentId: operation.parentId, contentFormat: note.format });
      edgesToAdd.push(containsEdge);
      graphDiff.push({ action: "add", edge: containsEdge, reason: "structural contains edge for add_node" });
      notePreviews.push({ nodeId: operation.id, domain: operation.domain, title: operation.title, mode: "create", contentFormat: note.format, markdown: note.markdown, html: note.html });
    }
    ...
    if (operation.type === "add_block_type") {
      ...
      blockTypesToAdd.push(blockTypeSummary(raw, operation.file));
    }
    if (operation.type === "propose_native_block") reviewItems.push({ type: "propose_native_block", title: operation.blockType || operation.typeName || operation.name, message: operation.reason || "Proposal only; no executable files will be written." });
  });

  (packageFiles.assetFiles || []).forEach((asset) => {
    assetsToAdd.push({ path: asset.vaultRelativePath, sourcePath: asset.packageRelativePath, mimeType: asset.mimeType, size: asset.size });
  ...
  return { filesToCreate, filesToModify, domainsToAdd, nodesToAdd, assetsToAdd, edgesToAdd, blockTypesToAdd, notePreviews, graphDiff, reviewItems, validation: { valid: validatedPackage.valid, errors: validatedPackage.errors, warnings: validatedPackage.warnings } };
}
