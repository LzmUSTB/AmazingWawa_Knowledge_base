...
export const STATIC_CONTEXT_FILES = {
  ...
  "WORKFLOW_GUIDE.md": markdown([
    ...
    "6. Every problem must include `mode`, prompt, answer, and solution.",
    "7. Use `mode: recall` for memory/review questions and `mode: practice` for worked-answer problems.",
    "8. The app records recall with Anki-like review. Practice stores only the current user answer and current correct/wrong result.",
    "9. AI generation is not built into the app; produce reviewed files only when the user asks.",
    "",
  ...
  "FORMAT_CONTRACT.md": markdown([
    ...
    "",
    "Problems must declare `mode: recall` or `mode: practice`. Do not infer missing mode in generated files.",
    "",
    "Do not add mistake tags, error categories, weakness categories, or AI judging fields.",
    "",
  ...
  "EXERCISE_SCHEMA.md": markdown([
    ...
    "",
    "Allowed problem modes: `recall`, `practice`.",
    "",
    "`recall` uses Anki-like review controls: Wrong, Hard, Good, Easy. It is for memory and quick conceptual checks.",
    "",
    "`practice` records the current user answer and a self-assessed result: Correct or Wrong. It does not use attempts, accuracy, attempt history, ease, interval, or due scheduling.",
    "",
    "Every problem requires `id`, `mode`, `type`, `difficulty`, `title`, `prompt`, `answer`, and `solution`. Problem ids must be unique within the ExerciseSet. `hints` is optional.",
    "",
    "Do not include mistake tags, error categories, weakness categories, or automatic judging fields.",
    "",
    "ExerciseSet import appends new problems to the existing node ExerciseSet and never overwrites existing problems. Duplicate ids with identical content may be skipped; duplicate ids with different content are conflicts.",
    "",
    "Wrong practice export is filter-based and intended to provide context for future AI-assisted exercise generation. AI generation itself is not built into the app yet.",
  ]),
