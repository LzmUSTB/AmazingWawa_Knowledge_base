...
(function () {
    ...
    function Drawer(scale, container, mode) {
        ...
        this.repaint = function () {
            ...
            // Wawa KB source-porting runtime fix:
            // In the original article these modes sit inside .dark_light_bg, so the
            // transparent canvas is visually backed by black. In isolated HTML-note
            // renderers, CSS canvas backgrounds can be overridden or ignored by the
            // embedding environment, which makes transparent regions appear white.
            // Fill the backing bitmap with the original dark visual context before
            // source drawing so destination-out feathering and lighter blending keep
            // the same readability as the source page.
            if (mode === "detector" ||
                mode === "rgb_filter" ||
                mode === "bayer" ||
                mode === "detector_rgbg" ||
